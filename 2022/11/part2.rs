...
const MODULUS: u32 = 2 * 3 * 5 * 7 * 11 * 13 * 17 * 19 * 23; // largest primorial number <= 2^32 - 1
	...
	.map(|record| {
		...
		let operation = lines
			...
			.pipe(|(op, arg)| -> Box<dyn Fn(u32) -> u32> {
				let op = match op {
					"+" => |a: u32, b| a.checked_add(b).unwrap() % MODULUS,
					"*" => |a: u32, b| {
						((a as u64).checked_mul(b as u64).unwrap() % MODULUS as u64) as u32
					},
					_ => unreachable!(),
				};
				if arg == "old" {
